...
export default function VideoURLsCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setSrcvideo(initialValues.srcvideo);
    setMP4(initialValues.MP4);
    setCMAF(initialValues.CMAF);
    setHLS(initialValues.HLS);
    setDASH(initialValues.DASH);
    setMSS(initialValues.MSS);
    setErrors({});
  };
  const validations = {
    guid: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new VideoURLs(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "VideoURLsCreateForm")}
      {...rest}
    >
      <TextField
        label="Guid"
